...
- **M5 — Métriques (en mémoire) & panels.** 🔵 *en cours.* `metrics.py` calculé **en mémoire** sur le snapshot + l'historique 48 h — ~192 frames, agrégations triviales, **pas de DB** (DuckDB ne paie qu'avec le long historique du M6). **Congestion ✅** : spreads **zone-level** (heatmap/leaderboard, inclut les splits intra-pays), rente **seulement où attribuable** (frontière = lien unique de sa paire de pays), indice de convergence 48 h ; lib **ECharts** introduite (wrapper maison) + dock de panneaux. **Sankey ✅** : flux nets **bipartite** (exporteur→importateur — acyclique pour ECharts, et `in−out` = position nette par pays). **Explorer ✅** : liste cherchable des frontières triée par flux courant + détail par frontière (flux commercial vs physique + overlay spread sur 48 h), **100% client-side** (réutilise `/api/interconnectors` + l'historique, 0 appel en plus) ; NTC/utilisation + duration curve = M6. **Reste** : Zone dashboard (prix/charge/mix — mix via `/public_power` à la demande). NTC/utilisation **seulement** si ENTSO-E (M6). Critères §4.2–4.5.
