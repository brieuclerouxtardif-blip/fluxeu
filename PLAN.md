...
- **M5 — Métriques (en mémoire) & panels.** 🔵 *en cours.* `metrics.py` calculé **en mémoire** sur le snapshot + l'historique 48 h — ~192 frames, agrégations triviales, **pas de DB** (DuckDB ne paie qu'avec le long historique du M6). **Congestion ✅** : spreads **zone-level** (heatmap/leaderboard, inclut les splits intra-pays), rente **seulement où attribuable** (frontière = lien unique de sa paire de pays), indice de convergence 48 h ; lib **ECharts** introduite (wrapper maison) + dock de panneaux. **Reste** : Sankey des flux nets, Interconnector explorer + page détail, Zone dashboard (mix via `/public_power` à la demande). NTC/utilisation **seulement** si ENTSO-E (M6). Critères §4.2–4.5.
