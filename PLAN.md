...
| GET | `/api/prices` · `/api/flows` | séries | ✅ M6 — séries depuis DuckDB (zone-level enrichi par ENTSO-E) |
| GET | `/api/analytics/coverage` · `/duration` · `/correlation` | objets | ✅ M6 — couverture, monotone, corrélation (SQL DuckDB) |
| GET | `/api/export.csv?...` | text/csv | ✅ M6 — download prix/flux |
| GET | `/api/alerts` | `AlertsSnapshot` | ✅ M7 — prix négatifs, pics, congestion, capacité (NTC) |
| GET | `/api/model/forward?zone=&horizon=` | `ForwardCurve` | ✅ M7 — forward seasonal-naive + spot réalisé |
...
- **M6 — Analytics + DuckDB + ENTSO-E upgrade.** 🟧 **Introduire DuckDB ici** (substrat d'accumulation durable + SQL, là où le long historique paie : duration curves / corrélations sur 30 j). Brancher `entsoe.py` si token (sélection auto via `/api/health`) → **NTC réels + flux zone→zone** (débloque utilisation + arcs par zone) ; export CSV ; tests pytest/Vitest, design pass. ✅ DoD global §10.
  - **État (parties indépendantes du token, faites)** : ✅ DuckDB (`store/duckdb_store.py` : schéma prix/flux, upsert idempotent, ingestion par sweep dans le scheduler + seed immédiat du cache 48 h au boot) ; ✅ endpoints `/api/prices`, `/api/flows`, `/api/analytics/{coverage,duration,correlation}`, `/api/export.csv` (monotone via window-func, corrélation via `corr()` SQL) ; ✅ panneau **Analytics** front (compare multi-zones, monotone de prix, heatmap de corrélation daltonien-safe, export CSV, sélecteur de fenêtre 24 h/7 j/30 j) ; ✅ `entsoe.py` **écrit & câblé** (graphe zone + EIC depuis `entsoe-py` `Area`/`NEIGHBOURS`, flux signés zone→zone, NTC, pacing + threadpool ; registry à sélection auto avec fallback Energy-Charts) + tests offline (graphe, transforms, fallback) ; ✅ pytest vert (44). ⏳ **Reste (token requis)** : vérifier le chemin live ENTSO-E (NTC réels + flux zone→zone end-to-end) une fois le token reçu ; Vitest non câblé (build `tsc`+`vite` vert en attendant).
- **M7 (option)** — Module modélisation (§4.7) + alertes (§4.8). 🟧 **Faits (indépendants du token)** : ✅ **Alertes/signaux** — `domain/alerts.py` + `/api/alerts` (prix négatifs, pics absolus **et** statistiques (z-score vs distribution 48 h de la zone), congestion (réutilise `border_spreads`), capacité quasi-pleine inerte sans NTC) ; front : badge d'en-tête (compte crit+warn, couleur = pire sévérité) + panneau **Alertes** (feed trié par sévérité). ✅ **Modélisation forward** — `domain/model.py` + `/api/model/forward?zone=&horizon=` : baseline **seasonal-naive** (profil horaire p10/p50/p90 en heure de Bruxelles, depuis l'historique DuckDB) + spot réalisé 48 h ; front : panneau **Modélisation** (bande p10–p90 + p50 pointillé vs réalisé). Placeholder explicite pour le vrai modèle merit-order / `peakero-forecaster` (même contrat d'overlay). pytest 53 vert, build front vert, panneaux vérifiés live. ⏳ **Reste** : brancher le simulateur réel ; sandbox de scénario (§4.7) ; couche carbone Electricity Maps (optionnelle).
